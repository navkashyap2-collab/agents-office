...
- **Never call, email, follow up with, or prepare pricing/proposal/opportunity work for a prospect whose real Sales Control Call Outcome, Sales Stage, or Lead Quality says stop** (`Not Interested`, `Wrong Number`, `Lost / Not Fit`, `Won`, `Do Not Contact`, `Existing / Duplicate`, or a soft decline — `Has Cleaner`/`Future Review` — not yet past its reactivation date). This is decided deterministically in code (`src/va-outcome-routing.ts`), never by judgment — see `90-Operations/va-outcome-routing.md` for the full real-value table before touching any prospect a VA has already contacted.
